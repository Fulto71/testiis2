...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsInterne" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsInterne.svc ou wsInterne.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsInterne : IwsInterne
    {
        ...
        //LISTE
        public string pvgChargerDansDataSetPourCombo(clsInterne Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsInterne clsInterne = new ZenithWebServeur.BOJ.clsInterne();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsInterne clsInterneDTO in Objet)
                //{

                string[] vlpChamp1 = new string[] { "CODEINTERNE", "LIBELLE" };
                DataSet.Tables.Add("TABLE");
                for (int i = 0; i < vlpChamp1.Length; i++)
                {
                    DataSet.Tables[0].Columns.Add(vlpChamp1[i]);
                }
                DataSet.Tables[0].Rows.Add("I", "INTERNE");
                DataSet.Tables[0].Rows.Add("E", "EXTERNE");

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                
                //DataSet = clsInterneWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
