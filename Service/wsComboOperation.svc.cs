...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsComboOperation" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsComboOperation.svc ou wsComboOperation.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsComboOperation : IwsComboOperation
    {
        ...
        //LISTE
        public string pvgChargerDansDataSetPourCombo(clsComboOperation Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsComboOperation clsComboOperation = new ZenithWebServeur.BOJ.clsComboOperation();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsComboOperation clsComboOperationDTO in Objet)
                //{

                string[] vlpChamp1 = new string[] { "CO_CODEOPERATION", "CO_LIBELLE" };
                DataSet.Tables.Add("TABLE");
                for (int i = 0; i < vlpChamp1.Length; i++)
                {
                    DataSet.Tables[0].Columns.Add(vlpChamp1[i]);
                }
                DataSet.Tables[0].Rows.Add("P", "+");
                DataSet.Tables[0].Rows.Add("M", "-");

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                
                //DataSet = clsComboOperationWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
