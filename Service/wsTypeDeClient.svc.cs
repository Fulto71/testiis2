...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsTypeDeClient" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsTypeDeClient.svc ou wsTypeDeClient.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsTypeDeClient : IwsTypeDeClient
    {
        ...
        //LISTE
        public string pvgChargerDansDataSetPourCombo(clsTypeDeClient Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsTypeDeClient clsTypeDeClient = new ZenithWebServeur.BOJ.clsTypeDeClient();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsTypeDeClient clsTypeDeClientDTO in Objet)
                //{

                string[] vlpChamp1 = new string[] { "TC_CODETYPEDECLIENT", "TC_LIBELLE" };
                DataSet.Tables.Add("TABLE");
                for (int i = 0; i < vlpChamp1.Length; i++)
                {
                    DataSet.Tables[0].Columns.Add(vlpChamp1[i]);
                }
                DataSet.Tables[0].Rows.Add("P", "PHYSIQUE");
                DataSet.Tables[0].Rows.Add("M", "MORALE");
                DataSet.Tables[0].Rows.Add("I", "INSTITUTION");

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                
                //DataSet = clsTypeDeClientWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
