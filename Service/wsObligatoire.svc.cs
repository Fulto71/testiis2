...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsObligatoire" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsObligatoire.svc ou wsObligatoire.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsObligatoire : IwsObligatoire
    {
        ...
        //LISTE
        public string pvgChargerDansDataSetPourCombo(clsObligatoire Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsObligatoire clsObligatoire = new ZenithWebServeur.BOJ.clsObligatoire();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsObligatoire clsObligatoireDTO in Objet)
                //{

                string[] vlpChamp1 = new string[] { "DC_CODEOBLIGATOIRE", "DC_OBLIGATOIRE" };
                DataSet.Tables.Add("TABLE");
                for (int i = 0; i < vlpChamp1.Length; i++)
                {
                    DataSet.Tables[0].Columns.Add(vlpChamp1[i]);
                }
                DataSet.Tables[0].Rows.Add("OUI", "OUI");
                DataSet.Tables[0].Rows.Add("NON", "NON");

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                
                //DataSet = clsObligatoireWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
