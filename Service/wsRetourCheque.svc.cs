...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsRetourCheque" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsRetourCheque.svc ou wsRetourCheque.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsRetourCheque : IwsRetourCheque
    {
        ...
        //LISTE
        public string pvgChargerDansDataSetPourCombo(clsRetourCheque Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsRetourCheque clsRetourCheque = new ZenithWebServeur.BOJ.clsRetourCheque();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsRetourCheque clsRetourChequeDTO in Objet)
                //{

                string[] vlpChamp1 = new string[] { "RC_CODERETOURCHEQUE", "RC_LIBELLE" };
                DataSet.Tables.Add("TABLE");
                for (int i = 0; i < vlpChamp1.Length; i++)
                {
                    DataSet.Tables[0].Columns.Add(vlpChamp1[i]);
                }
                DataSet.Tables[0].Rows.Add("PAYÉ", "PAYÉ");
                DataSet.Tables[0].Rows.Add("IMPAYÉ", "IMPAYÉ");

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                
                //DataSet = clsRetourChequeWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
