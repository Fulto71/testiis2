...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsSens" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsSens.svc ou wsSens.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsSens : IwsSens
    {
        ...
        //LISTE
        public string pvgChargerDansDataSetPourCombo(clsSens Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsSens clsSens = new ZenithWebServeur.BOJ.clsSens();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsSens clsSensDTO in Objet)
                //{

                string[] vlpChamp1 = new string[] { "CODESENS", "LIBELLESENS" };
                DataSet.Tables.Add("TABLE");
                for (int i = 0; i < vlpChamp1.Length; i++)
                {
                    DataSet.Tables[0].Columns.Add(vlpChamp1[i]);
                }
                DataSet.Tables[0].Rows.Add("D", "DEBIT");
                DataSet.Tables[0].Rows.Add("C", "CREDIT");

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                
                //DataSet = clsSensWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
