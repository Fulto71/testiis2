...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsCoupure" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsCoupure.svc ou wsCoupure.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsCoupure : IwsCoupure
    {
        private clsDonnee _clsDonnee = new clsDonnee();
        private clsMessagesWSBLL clsMessagesWSBLL = new clsMessagesWSBLL();
        private clsCoupureWSBLL clsCoupureWSBLL = new clsCoupureWSBLL();
        ...
        //AJOUT
        public string pvgAjouter(clsCoupure Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsCoupure clsCoupure = new ZenithWebServeur.BOJ.clsCoupure();
          ...
          //DataSet = TestChampObligatoireInsert(Objet);
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsCoupure clsCoupureDTO in Objet)
                //{

               
                clsCoupure.CB_LIBELLE = Objet.CB_LIBELLE.ToString();
                clsCoupure.CB_VALEUR = Double.Parse(Objet.CB_VALEUR.ToString());
                clsCoupure.CB_STATUT = Objet.CB_STATUT.ToString();

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;

                clsObjetRetour.SetValue(true, clsCoupureWSBLL.pvgAjouter(clsDonnee, clsCoupure, clsObjetEnvoi));
        ...
        //MODIFICATION
        public string pvgModifier(clsCoupure Objet)
        {
            DataSet DataSet = new DataSet();
            DataTable dt = new DataTable("TABLE");
            dt.Columns.Add(new DataColumn("SL_CODEMESSAGE", typeof(string)));
           ...
           //DataSet = TestChampObligatoireUpdate(Objet);
            ...
            try
            {
                //clsDonnee.pvgConnectionBase();
                clsDonnee.pvgDemarrerTransaction();
                clsObjetEnvoi.OE_PARAM = new string[] { Objet.CB_CODECOUPURE };

                //foreach (ZenithWebServeur.DTO.clsCoupure clsCoupureDTO in Objet)
                //{

                clsCoupure.CB_CODECOUPURE = Objet.CB_CODECOUPURE.ToString();
                clsCoupure.CB_LIBELLE = Objet.CB_LIBELLE.ToString();
                clsCoupure.CB_VALEUR = Double.Parse(Objet.CB_VALEUR.ToString());
                clsCoupure.CB_STATUT = Objet.CB_STATUT.ToString();
                ...
                clsObjetRetour.SetValue(true, clsCoupureWSBLL.pvgModifier(clsDonnee, clsCoupure, clsObjetEnvoi));
                if (clsObjetRetour.OR_BOOLEEN)
                {
                    DataSet = new DataSet();
                    DataRow dr = dt.NewRow();
        ...
        //SUPPRESSION
        public string pvgSupprimer(clsCoupure Objet)
        {
            DataSet DataSet = new DataSet();
            DataTable dt = new DataTable("TABLE");
            dt.Columns.Add(new DataColumn("SL_CODEMESSAGE", typeof(string)));
            ...
            try
            {
                ...
                clsObjetRetour.SetValue(true, clsCoupureWSBLL.pvgSupprimer(clsDonnee, clsObjetEnvoi));
                if (clsObjetRetour.OR_BOOLEEN)
                {
                    DataSet = new DataSet();
                    DataRow dr = dt.NewRow();
        ...
        //LISTE
        public string pvgChargerDansDataSet(clsCoupure Objet)
        {
            DataSet DataSet = new DataSet();
            DataTable dt = new DataTable("TABLE");
            dt.Columns.Add(new DataColumn("SL_CODEMESSAGE", typeof(string)));
            ...
            try
            {
                ...
                DataSet = clsCoupureWSBLL.pvgChargerDansDataSet(clsDonnee, clsObjetEnvoi);
