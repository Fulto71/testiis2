...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsTypedoperation" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsTypedoperation.svc ou wsTypedoperation.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsTypedoperation : IwsTypedoperation
    {
        ...
        //COMBO
        public string pvgChargerDansDataSetPourCombo(clsTypedoperation Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsTypedoperation clsTypedoperation = new ZenithWebServeur.BOJ.clsTypedoperation();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsTypedoperation clsTypedoperationDTO in Objet)
                //{

                string[] vlpChamp1 = new string[] { "TO_CODETYPEDOPERATION", "TO_LIBELLE" };
                DataSet.Tables.Add("TABLE");
                for (int i = 0; i < vlpChamp1.Length; i++)
                {
                    DataSet.Tables[0].Columns.Add(vlpChamp1[i]);
                }
                DataSet.Tables[0].Rows.Add("I", "IMPORTATION");
                DataSet.Tables[0].Rows.Add("E", "EXPORTATION");

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                
                //DataSet = clsTypedoperationWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
