...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsRegimeMatrimonial" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsRegimeMatrimonial.svc ou wsRegimeMatrimonial.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsRegimeMatrimonial : IwsRegimeMatrimonial
    {
        ...
        //LISTE
        public string pvgChargerDansDataSetPourCombo(clsRegimeMatrimonial Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsRegimeMatrimonial clsRegimeMatrimonial = new ZenithWebServeur.BOJ.clsRegimeMatrimonial();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsRegimeMatrimonial clsRegimeMatrimonialDTO in Objet)
                //{

                string[] vlpChamp1 = new string[] { "RM_CODEREGIMEMATRIMONIAL", "RM_LIBELLE" };
                DataSet.Tables.Add("TABLE");
                for (int i = 0; i < vlpChamp1.Length; i++)
                {
                    DataSet.Tables[0].Columns.Add(vlpChamp1[i]);
                }
                DataSet.Tables[0].Rows.Add("C", "COMMUNAUTE DE BIENS");
                DataSet.Tables[0].Rows.Add("S", "SEPARATION DE BIENS");

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                
                //DataSet = clsRegimeMatrimonialWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
