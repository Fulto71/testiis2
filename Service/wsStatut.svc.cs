...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsStatut" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsStatut.svc ou wsStatut.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsStatut : IwsStatut
    {
        ...
        //LISTE
        public string pvgChargerDansDataSetPourCombo(clsStatut Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsStatut clsStatut = new ZenithWebServeur.BOJ.clsStatut();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsStatut clsStatutDTO in Objet)
                //{

                string[] vlpChamp1 = new string[] { "S_CODESTATUT", "S_LIBELLE" };
                DataSet.Tables.Add("TABLE");
                for (int i = 0; i < vlpChamp1.Length; i++)
                {
                    DataSet.Tables[0].Columns.Add(vlpChamp1[i]);
                }
                DataSet.Tables[0].Rows.Add("C", "Clôturés");
                DataSet.Tables[0].Rows.Add("S", "En suspension");
                DataSet.Tables[0].Rows.Add("E", "En cours d'exécution");

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                
                //DataSet = clsStatutWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
        ...
        //LISTE
        public string pvgChargerDansDataSetPourCombo2(clsStatut Objet)
        {
            DataSet DataSet = new DataSet();
            DataTable dt = new DataTable("TABLE");
            dt.Columns.Add(new DataColumn("SL_CODEMESSAGE", typeof(string)));
