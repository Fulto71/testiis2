...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsEtatcredit" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsEtatcredit.svc ou wsEtatcredit.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsEtatcredit : IwsEtatcredit
    {
        private clsDonnee _clsDonnee = new clsDonnee();
        private clsMessagesWSBLL clsMessagesWSBLL = new clsMessagesWSBLL();
        private clsEtatcreditWSBLL clsEtatcreditWSBLL = new clsEtatcreditWSBLL();
        ...
        public string pvgChargerDansDataSet(clsEtatcredit Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsEtatcredit clsEtatcredit = new ZenithWebServeur.BOJ.clsEtatcredit();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsEtatcredit clsEtatcreditDTO in Objet)
                //{

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                //}
                DataSet =  clsEtatcreditWSBLL.pvgChargerDansDataSet(clsDonnee, clsObjetEnvoi);
        ...
        public string pvgChargerDansDataSetPourCombo(clsEtatcredit Objet)
        {
            DataSet DataSet = new DataSet();
            DataTable dt = new DataTable("TABLE");
            dt.Columns.Add(new DataColumn("SL_CODEMESSAGE", typeof(string)));
            ...
            try
            {
                ...
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                //}
                DataSet = clsEtatcreditWSBLL.pvgChargerDansDataSetPourCombo(clsDonnee, clsObjetEnvoi);
                if (DataSet.Tables[0].Rows.Count > 0)
                {
                    DataSet.Tables[0].Columns.Add(new DataColumn("SL_CODEMESSAGE", typeof(string)));
                    DataSet.Tables[0].Columns.Add(new DataColumn("SL_RESULTAT", typeof(string)));
