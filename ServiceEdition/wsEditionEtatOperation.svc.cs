...
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsEditionEtatOperation" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsEditionEtatOperation.svc ou wsEditionEtatOperation.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsEditionEtatOperation : IwsEditionEtatOperation
    {
        private clsDonnee _clsDonnee = new clsDonnee();
        private clsMessagesWSBLL clsMessagesWSBLL = new clsMessagesWSBLL();
        private clsEditionEtatOperationWSBLL clsEditionEtatOperationWSBLL = new clsEditionEtatOperationWSBLL();
        ...
        public string pvgInsertIntoDataSetListeEtat(clsEditionEtatOperation Objet)
        {
            ...
            ZenithWebServeur.BOJ.clsEditionEtatOperation clsEditionEtatOperation = new ZenithWebServeur.BOJ.clsEditionEtatOperation();
            ...
            try
            {
                ...
                //foreach (ZenithWebServeur.DTO.clsEditionEtatOperation clsEditionEtatOperationDTO in Objet)
                //{

                clsObjetEnvoi.OE_A = Objet.clsObjetEnvoi.OE_A;
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                //}
                DataSet = clsEditionEtatOperationWSBLL.pvgInsertIntoDataSetListeEtat(clsDonnee, clsObjetEnvoi);
        ...
        ///<summary>Cette fonction permet de d'executer une requete SELECT dans la base de donnees </summary>
        ///<author>Home Technology</author>
        public string pvgInsertIntoDatasetEtatSuiviEvenement(clsEditionEtatOperation Objet)
        {
            DataSet DataSet = new DataSet();
            DataTable dt = new DataTable("TABLE");
            dt.Columns.Add(new DataColumn("SL_CODEMESSAGE", typeof(string)));
            ...
            try
            {
                ...
                clsObjetEnvoi.OE_PARAM = new string[] {
                        ...
                        Objet.DATEDEBUT,
                        Objet.DATEFIN,
                        Objet.PO_CODEPROCEDUREAUTOMATIQUE,
                ...
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                //}
                DataSet = clsEditionEtatOperationWSBLL.pvgInsertIntoDatasetEtatSuiviEvenement(clsDonnee, clsObjetEnvoi);
                if (DataSet.Tables[0].Rows.Count > 0)
                {
                    // json = JsonConvert.SerializeObject(DataSet, Formatting.Indented);
        ...
        ///<summary>Cette fonction permet de d'executer une requete SELECT dans la base de donnees </summary>
        ///<author>Home Technology</author>
        public string pvgInsertIntoDataSetDocumentEtatOperation(clsEditionEtatOperation Objet)
        {
            DataSet DataSet = new DataSet();
            DataTable dt = new DataTable("TABLE");
            dt.Columns.Add(new DataColumn("SL_CODEMESSAGE", typeof(string)));
            ...
            try
            {
                ...
                clsObjetEnvoi.OE_Y = Objet.clsObjetEnvoi.OE_Y;
                //}
                DataSet = clsEditionEtatOperationWSBLL.pvgInsertIntoDataSetDocumentEtatOperation(clsDonnee, clsObjetEnvoi);
                if (DataSet.Tables[0].Rows.Count > 0)
                {
                    // json = JsonConvert.SerializeObject(DataSet, Formatting.Indented);
                    string reportPath = "~/Etats/" + Objet.ET_DOSSIER;
